...
local function DecorateNomi()
	...
	for j = 1, #IngredientOrder do
		...
		if count >= 5 then -- we have enough of an ingredient for nomi to display it
			...
			local button = GossipFrame_GetTitleButton(i) --_G[buttonName]
			local buttonIcon = GossipFrame_GetTitleButton(i).Icon --_G[buttonName .. 'GossipIcon'] -- check that the icon is for a work order, otherwise we might overwrite a quest button or something
			if button and button:IsShown() and buttonIcon and buttonIcon:GetTexture():lower() == 'interface/gossipframe/workordergossipicon' then --changed here too: the \\ to /
